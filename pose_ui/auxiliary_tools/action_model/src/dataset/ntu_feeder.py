...
class NTU_Feeder(Dataset):
    def __init__(self, phase, dataset_path, inputs, num_frame, connect_joint, debug, **kwargs):
        ...
        data_path = 'C:/Users/HUST/Desktop/action_recognition/EfficientGCNv1-master/npy_dataset/original2/ntu-xsub120/eval_data.npy'
        #data_path = '{}/{}_data.npy'.format(dataset_path, phase)
        #label_path = '{}/{}_label.pkl'.format(dataset_path, phase)
        label_path = r'C:\Users\HUST\Desktop\action_recognition\EfficientGCNv1-master\npy_dataset\original2\ntu-xsub120\eval_label.pkl'
        try:
            self.data = np.load(data_path, mmap_mode='r')
            with open(label_path, 'rb') as f:
                self.name, self.label, self.seq_len = pickle.load(f, encoding='latin1')
            #self.name, self.label, self.seq_len=([0],[0],[0])
        except:
            logging.info('')
